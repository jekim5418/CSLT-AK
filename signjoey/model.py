...
from signjoey.ensemble import EnsembleEmbeddings, EnsembleGlosses, EnsembleSpatialEmbeddings, EnsembleTransformerDecoder, EnsembleTransformerEncoder
...
#### pkl load for keypoint ####
def pkl_load(phase, dir):
    pkl_dir = f"data/{dir}_pkl/"
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        #### keypoint and token ####
        self.train_keypoint_info ={}
        self.dev_keypoint_info ={}
        self.test_keypoint_info ={}

        self.train_token_info = []
        self.dev_token_info = []
        self.test_token_info = []
    ...
    def forward(
        ...
        """
        if self.encoder.__class__.__name__ == 'EnsembleTransformerEncoder':
    ...
    def encode(
        ...
        """
        return self.encoder(
            embed_src=self.sgn_embed(x=sgn, mask=sgn_mask, keypoint=self.batch_keypoint),
    ...
    def get_loss_for_batch(
        ...
        if phase == 'train':
            if not self.train_keypoint_info:
                print('load keypoint and token for train data')
                self.train_keypoint_info = pkl_load(phase, 'keypoint')
                self.train_token_info = pkl_load(phase, f'action_token')
            keypoint_info = self.train_keypoint_info
            token_info = self.train_token_info
        elif phase == 'dev':
            if not self.dev_keypoint_info:
                print('load keypoint and token for dev data')
                self.dev_keypoint_info = pkl_load(phase, 'keypoint')
                self.dev_token_info = pkl_load(phase, f'action_token')
            keypoint_info = self.dev_keypoint_info
            token_info = self.dev_token_info
        elif phase == 'test':
            if not self.test_keypoint_info:
                print('load keypoint and token for test data')
                self.test_keypoint_info = pkl_load(phase, 'keypoint')
                self.test_token_info = pkl_load(phase,f'action_token')
            keypoint_info = self.test_keypoint_info
            token_info = self.test_token_info

        # for keypoint
        pad = batch.sgn.shape[1]
        arr = list(map(lambda b_s: keypoint_info[b_s.split('/')[1]].reshape(-1,148), batch.sequence))
        arr_pad = list(map(lambda a: np.pad(a, ((0,pad-len(a)),(0,0)), 'constant', constant_values=0), arr))
        self.batch_keypoint = torch.from_numpy(np.array(arr_pad)).float().cuda()
        
        # for token
        self.batch_num_token = list(map(lambda b_s: token_info[b_s.split('/')[1]]['num_token'], batch.sequence))
        self.batch_breakpoint = list(map(lambda b_s: token_info[b_s.split('/')[1]]['breakpoint'], batch.sequence))
...
def build_model(
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        ...
        print('not transformer encoder')
    if do_recognition:
        if ensemble:
            gloss_output_layer: Union[nn.Linear, None] = EnsembleGlosses(
                N = ensembleN,
                enc_output_size = encoder.output_size,
                gls_output_size = len(gls_vocab)
            )
        else:
