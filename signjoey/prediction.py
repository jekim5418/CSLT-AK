...
from  signjoey.ensemble import BuildEnsebleCKPT
...
def test(
    cfg, ckpt: str, output_path: str = None, logger: logging.Logger = None
) -> None:
    """
    ...
    if isinstance(cfg['training']['model_dir'], list):
        ensemble=True
    ...
    if ensemble:
        for i in range(ensembleN):
            model.gloss_output_layer.Glosses[i].weight = torch.nn.Parameter(model_checkpoint['model_state'][f'gloss.Glosses.{i}_output_layer.weight'])
            model.gloss_output_layer.Glosses[i].bias = torch.nn.Parameter(model_checkpoint['model_state'][f'gloss.Glosses.{i}_output_layer.bias'])
