    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                # reactivate training
                # create a Batch object from torchtext.legacy batch
                batch = Batch(
                    is_train=True,
                    torch_batch=batch,
...
def train(cfg_file: str) -> None:
    ...
    with open(trainer.model_dir + "/config.yaml", "w", encoding="utf-8") as ymlfile:
    ...
    del trainer
    test(cfg, ckpt=ckpt, output_path=output_path, logger=logger)
